...
export const getProjectBySlug = async (slug: string): Promise<Project> => {
    const res = await fetch(buildApiUrl(`/api/projects/${slug}`))
...
export const createProject = async (data: ProjectInput): Promise<Project> => {
    const res = await fetchWithAuth(buildApiUrl("/api/projects"), {
        method: "POST",
...
export const updateProject = async (slug: string, data: ProjectInput): Promise<Project> => {
    const res = await fetchWithAuth(buildApiUrl(`/api/projects/${slug}`), {
        method: "PUT",
        body: JSON.stringify(data),
    })
...
export const deleteProject = async (slug: string): Promise<void> => {
    const res = await fetchWithAuth(buildApiUrl(`/api/projects/${slug}`), { method: "DELETE" })
